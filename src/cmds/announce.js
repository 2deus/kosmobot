...
module.exports = {
    ...
    async execute(intrc, client) {
        if (!intrc.member.permissions.has(PermissionsBitField.Flags.ManageMessages)) {
            await intrc.reply({ content: `you do not have permission to run this command .`, ephemeral: true});
            return;
        }
        ...
        if (logChannel && logChannel.isTextBased()) logChannel.send({ embeds: [embed] })
            else await intrc.editReply({ content: `message sent but logging failed. check whether <#${process.env.LOG_ID}> exists`, ephemeral: true});
        return;
    }
};