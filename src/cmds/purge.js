...
module.exports = {
    ...
    async execute(intrc, client) {
        if (!intrc.member.permissions.has(PermissionsBitField.Flags.ManageMessages)) {
            await intrc.reply({ content: `you do not have permissions 2 run this command .`, ephemeral: true});
            return;
        }
        ...
        try {
            ...
            if (logChannel && logChannel.isTextBased()) logChannel.send({ embeds: [embed] });
                    ...
                    await intrc.reply({ content: `<<<<<<<<WARNING>>>>>>>> command executed but logging failed. check whether <#${process.env.LOG_ID}> exists`, ephemeral: true});
                    return;
                }
            ...
            console.error(e);
            await intrc.reply({ content: 'error trying 2 delete', ephemeral: true });
            return;
        }
        return;
    }
